    ...
    <div className="bg-white rounded-card border border-card-border shadow-card p-5">
      ...
      {hasCritical && criticalLegendLabel && (
        <div className="flex items-center gap-1.5 mb-3">
          ...
          <span className="text-xs text-gray-400">{criticalLegendLabel}</span>
        </div>
      )}

      {data.length === 0 && <p className="text-sm text-gray-400 py-6 text-center">{emptyMessage}</p>}
        ...
        <div className="flex flex-col gap-2 mt-3">
          {data.map((d, i) => {
            ...
            return (
              ...
              >
                <span className="w-24 shrink-0 text-xs text-gray-500 truncate text-right">{d.label}</span>
                ...
                <span className="w-14 shrink-0 text-xs font-medium text-gray-600 tabular-nums">
                  {valueFormatter(d.value)}
                </span>

                {hovered === i && (
                  <div
                    className="absolute left-24 -top-8 z-10 px-2.5 py-1.5 rounded-md bg-gray-900 text-white text-xs whitespace-nowrap shadow-lg pointer-events-none"
                    role="tooltip"
                  >
            ...
            <tbody>
              {data.map((d) => (
                <tr key={d.label} className="border-t border-card-border">
                  ...
                  <td className="py-2 text-right tabular-nums text-gray-700">{valueFormatter(d.value)}</td>
                </tr>
              ))}
            </tbody>
          </table>
